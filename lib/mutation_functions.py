...
def do_basic_mutations(
    ...
    """Mutates image inputs with white noise.  添加白噪音
  ...
  Args:
    ...
    mutations_count: Integer representing number of mutations to do in  同时变异的数量
      parallel.
  ...
  """
    ...
    if len(corpus_element.data) > 1:
        ...
        image_batch = np.tile(image, [mutations_count, 1, 1, 1])  # 复制图片
    else:
    ...
    if constraint is not None:
        ...
        # when added to 'noise'
        ancestor, _ = corpus_element.oldest_ancestor()  # 最远祖先
        ...
        noise = np.clip(cumulative_noise, a_min=-constraint, a_max=constraint)  # 将噪音限制在a_min和a_max之间
