...
x_ = tf.placeholder(tf.float32, shape=(None, 28, 28, 1), name="x_")

image_input_tensor = tf.identity(x_, name="image_input_tensor")  # 与输入图片的shape一致
...
input_layer = tf.layers.flatten(image_input_tensor, name='input_layer')  # 对图片进行展平操作
hidden_layer_1 = tf.layers.dense(input_layer, 200, tf.nn.relu, kernel_initializer=init_func, name='hidden_layer_1')  # 添加全连接层, 输入，输出个数，激活函数，初始化权重
hidden_layer_2 = tf.layers.dense(hidden_layer_1, 100, tf.nn.relu, kernel_initializer=init_func, name='hidden_layer_2')  # 添加全连接层，100个输出
output_layer = tf.layers.dense(
    hidden_layer_2, 10, activation=None, kernel_initializer=init_func, name='output_layer')  # 添加全连接层
# return input_layer, hidden_layer_1, hidden_layer_2, output_layer, image_input_tensor
