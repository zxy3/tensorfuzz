...
tf.flags.DEFINE_string(
    "checkpoint_dir", "/tmp/quantizedfuzzer", "Dir containing checkpoints of model to fuzz."
)
tf.flags.DEFINE_string(
    "output_path", "/tmp/output", "Where to write the satisfying output."
)
...
tf.flags.DEFINE_float(
    "ann_threshold",
    1.0,
    "Distance below which we consider something new coverage.",  # 低于此值认为是新覆盖
)
...
def main(_):
    ...
    with tf.Session() as sess:
        ...
        fetch_function = fuzz_utils.build_fetch_function(sess, tensor_map)  # 获取模型信息

        size = FLAGS.mutations_per_corpus_item

        def mutation_function(elt):  # elt为一个元素
            """Mutates the element in question."""
            return do_basic_mutations(elt, size, FLAGS.perturbation_constraint)  # 一个元素多次变异，返回多个变异后的数据
