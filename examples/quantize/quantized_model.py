...
tf.flags.DEFINE_string(
    "checkpoint_dir",
    "/tmp/quantizedfuzzer",
    "The overall dir in which we store experiments",  # 存储实验数据
)
tf.flags.DEFINE_string(
...
def weight_variable(shape):
    """Construct variable for fully connected weights."""  # 初始化权重
    initial = tf.truncated_normal(shape, stddev=0.01)  # 从截断的正态分布中输出随机值
    return tf.Variable(initial)


def bias_variable(shape):
    """Construct variable for fully connected biases."""  # 初始化偏置值
...
def main(_):
    ...
    images = tf.identity(images)  # Return a tensor with the same shape and contents as input.
